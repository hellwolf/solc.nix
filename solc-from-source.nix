...
let
  ...
  solc = gccStdenv.mkDerivation rec {
    ...
    src = fetchzip {
      url = "https://github.com/ethereum/solidity/releases/download/v${solc_ver}/solidity_${solc_ver}.tar.gz";
      inherit sha256;
    };
    ...
    doInstallCheck = true;
    postInstall = ''
      mv --force $out/bin/solc $out/bin/solc-${solc_ver}
    '';
    installCheckPhase = ''
      $out/bin/solc-${solc_ver} --version > /dev/null
    '';
